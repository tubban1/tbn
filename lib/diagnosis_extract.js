...
export async function extractDiagnosisProfile(sessionId, latestUserMessage) {
  ...
  try {
    ...
    if (!API_KEY) {
      throw new Error('VECTORENGINE_GEMINI_KEY is not configured');
    }
...
请最终输出并且仅输出一个严格合规的 JSON 对象（如使用 markdown 包裹，请确保可以 parse），格式如下：
{
  ...
  "status": "collecting_info" // 如果 completeness >= 80，你可以选择升级为 "diagnosing"，否则保持 "collecting_info"
    ...
    const response = await axios.post(
      `${API_BASE}/chat/completions`,
      {
        ...
        messages: [
          ...
          { role: 'user', content: '请提取并更新企业画像事实。' }
        ],
        temperature: 0.1
      },
      {
    ...
    console.error(`[Diagnosis Extract] Async extraction failed for session: ${sessionId}:`, err);
    try {
      ...
      console.error('[Diagnosis Extract] Failed to update status to updated:', dbErr);
    }
  }
}
...
export async function extractDiagnosisProfileLocally(sessionId, latestUserMessage) {
  try {
    ...
    console.error('[Diagnosis Local Extract] Error:', err);
  }
}

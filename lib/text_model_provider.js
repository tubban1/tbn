...
function getTextProvider() {
  return (process.env.TEXT_MODEL_PROVIDER || 'vectorengine').toLowerCase();
}
...
function getVectorEngineConfig() {
  ...
  return {
    ...
    chatBaseUrl: (process.env.VECTORENGINE_API_BASE || 'https://api.frimodel.com/v1').replace(/\/$/, ''),
    model: process.env.PROMPT_MODEL || process.env.TEXT_MODEL || 'gemini-3.8-flash'
  };
}

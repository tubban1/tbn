...
3. **纯净背景与无字版方案保障（重要排版分支 🚫📝）**：
   - 在生成的 3 个风格提示词中，**必须保证包含至少 1 个“纯净无字版（无文案、无任何字母或排版）”的纯自然风光、纯插画或纯人物背景方案**！
   - 在该“无字版”方案的提示词中，应当明确使用 \`no text, no letters, no typography, textless, no labels, no watermark, pure scenery background\` 等反向标签，以确保图像画面绝无杂乱字符，方便用户将其作为纯净底图，或方便用户后续在图片上自己添加自定义文案。
4. 同时输出对应的“中文专业提示词(promptZh)”和“英文生图指令(prompt)”。“中文专业提示词”应当极具画面感与专业度。
5. 请以清晰的 JSON 数组格式直接返回，请确保仅返回 JSON，不需要其他解释文字，且格式如下：
[
  {

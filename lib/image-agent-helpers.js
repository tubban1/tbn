...
// Premium image model (IMAGE2) config & credit cost
export const PREMIUM_CREDITS_PER_IMAGE = 20;

export function getImageModelConfig(model) {
  if (model === 'image2') {
    const base = process.env.IMAGE2_API_BASE || process.env.IAMGE2_API_BASE || process.env.VECTORENGINE_API_BASE || 'https://api.vectorengine.cn/v1';
    const key = process.env.IMAGE2_KEY;
    const name = process.env.IMAGE2_MODEL || 'gpt-image-2-c';
    if (!key) {
      throw new Error('IMAGE2_KEY is not configured in .env');
    }
    return { apiBase: base, apiKey: key, model: name, isPremium: true, creditsPerImage: PREMIUM_CREDITS_PER_IMAGE };
  }
  // Default: standard model
  const base = process.env.VECTORENGINE_API_BASE || 'https://api.vectorengine.cn/v1';
  const key = process.env.VECTORENGINE_API_KEY;
  const name = process.env.IMAGE_MODEL || 'gpt-image-2';
  return { apiBase: base, apiKey: key, model: name, isPremium: false, creditsPerImage: null }; // null = use size-based calc
}

export function getAvailableModels() {
  const models = [
    { id: 'standard', label: '标准绘图', creditsHint: '8-25 积分/张（按尺寸）' }
  ];
  if (process.env.IMAGE2_KEY) {
    models.push({ id: 'image2', label: '高级绘图', creditsHint: `${PREMIUM_CREDITS_PER_IMAGE} 积分/张` });
  }
  return models;
}
